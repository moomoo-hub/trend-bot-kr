...
def run_with_retry(func, step_name: str):
    ...
    for i, delay in enumerate([0] + RETRY_DELAYS):
        try:
            if delay > 0:
                print(f"  [재시도] {delay}초 후 {step_name} 재시도...", flush=True)
                time.sleep(delay)
            return func()
        except Exception as e:
            if i == MAX_RETRIES:
                error_msg = str(e)[:100]
                print(f"  [실패] {step_name} {MAX_RETRIES}회 재시도 후 실패: {error_msg}", flush=True)
                send_error_report(step_name, error_msg)
                return None
            print(f"  [오류] {step_name} 시도 {i+1} 실패: {str(e)[:50]}...", flush=True)
...
def run_cycle() -> None:
    ...
    if hour < 9 or hour >= 22:
        print(f"\n[스케줄러] 현재 {hour:02d}시 - 운영 시간 외(09:00~21:59). 스킵.", flush=True)
        return

    print("\n" + "="*60, flush=True)
    print("트렌드봇 사이클 시작", flush=True)
    print("="*60, flush=True)
    ...
    if not keywords:
        print("\n[사이클] 키워드 수집 실패. 다음 주기에 재시도.", flush=True)
        return
    ...
    try:
        ...
        print(f"  [경고] 키워드 선별 실패: {e}. Fallback 사용.", flush=True)
        selected = keywords[:3]

    if not selected:
        print("\n[사이클] 키워드 선별 실패. 다음 주기에 재시도.", flush=True)
        return
    ...
    if not posts:
        print("\n[사이클] 게시글 작성 실패. 다음 주기에 재시도.", flush=True)
        return
    ...
    print("\n" + "="*60, flush=True)
    if success:
        print("[완료] 사이클 완료 (텔레그램 발송 완료)", flush=True)
    else:
        print("[경고] 사이클 완료 (일부 오류 발생)", flush=True)
    print("="*60 + "\n", flush=True)
